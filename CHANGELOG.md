...
## 2026-05-05 (Fork — ESP32 / ESP32-C3 board support)

### Feature:

* Added `esp32_DevKitC` board support for `edge_agent`: board metadata, empty peripheral/device definitions (no onboard RGB strip), GPIO-based blue LED setup on GPIO 2, and a `sdkconfig.defaults.board` that disables SPIRAM, selects 4 MB DIO flash, and tunes memory budgets for the classic dual-core ESP32 without PSRAM.

* Added `esp32c3_DevKitM` board support for `edge_agent`: board metadata, WS2812 RGB LED device on GPIO 8 via RMT (DMA disabled — C3 RMT has no DMA), matching peripheral definitions, and a `sdkconfig.defaults.board` tuned for the RISC-V single-core C3 (160 MHz, 4 MB flash, no PSRAM).

* Added `partitions_4MB.csv` partition table that fits within 4 MB flash: drops `ota_1` and the `emote` SPIFFS partition, providing a single `ota_0` app slot (~1.5 MB) and a FAT `storage` partition (256 KB).

* Updated `sdkconfig.defaults` with comments clarifying that SPIRAM configuration options are board-specific defaults that can be overridden per-board; no-PSRAM boards set `CONFIG_SPIRAM=n` in their own `sdkconfig.defaults.board`.
