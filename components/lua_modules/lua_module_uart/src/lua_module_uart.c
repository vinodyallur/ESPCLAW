...
#define LUA_MODULE_UART_METATABLE       "uart.port"
#define LUA_MODULE_UART_RX_BUF_SIZE     1024
#define LUA_MODULE_UART_TX_BUF_SIZE     0
#define LUA_MODULE_UART_MAX_READ_LEN    4096
#define LUA_MODULE_UART_MAX_LINE_LEN    1024
...
static lua_module_uart_ud_t *lua_module_uart_get_ud(lua_State *L, int idx)
{
    lua_module_uart_ud_t *ud = (lua_module_uart_ud_t *)luaL_checkudata(
        L, idx, LUA_MODULE_UART_METATABLE);
    if (!ud || !ud->installed) {
...
static TickType_t lua_module_uart_timeout_ticks(lua_State *L, int idx)
{
...
static uart_word_length_t lua_module_uart_parse_data_bits(lua_State *L, int bits)
{
    switch (bits) {
    case 5:
...
static uart_parity_t lua_module_uart_parse_parity(lua_State *L, const char *s)
{
    if (strcmp(s, "none") == 0) {
...
static uart_stop_bits_t lua_module_uart_parse_stop_bits(lua_State *L, int bits)
{
    switch (bits) {
    case 1:
...
static int lua_module_uart_new(lua_State *L)
{
    ...
    if (!lua_isnoneornil(L, 5)) {
        ...
        if (!lua_isnil(L, -1)) {
            data_bits = lua_module_uart_parse_data_bits(L, (int)luaL_checkinteger(L, -1));
        }
        lua_pop(L, 1);

        lua_getfield(L, 5, "parity");
        if (!lua_isnil(L, -1)) {
            parity = lua_module_uart_parse_parity(L, luaL_checkstring(L, -1));
        }
        lua_pop(L, 1);

        lua_getfield(L, 5, "stop_bits");
        if (!lua_isnil(L, -1)) {
            stop_bits = lua_module_uart_parse_stop_bits(L, (int)luaL_checkinteger(L, -1));
        }
        lua_pop(L, 1);
    }
    ...
    esp_err_t err = uart_driver_install(port,
                                        LUA_MODULE_UART_RX_BUF_SIZE,
                                        LUA_MODULE_UART_TX_BUF_SIZE,
                                        0, NULL, 0);
    if (err != ESP_OK) {
    ...
    lua_module_uart_ud_t *ud = (lua_module_uart_ud_t *)lua_newuserdata(
        L, sizeof(*ud));
    ud->port = port;
    ud->installed = true;
...
{
    lua_module_uart_ud_t *ud = lua_module_uart_get_ud(L, 1);
    lua_Integer len = luaL_checkinteger(L, 2);
    TickType_t ticks = lua_module_uart_timeout_ticks(L, 3);

    if (len <= 0 || len > LUA_MODULE_UART_MAX_READ_LEN) {
        return luaL_error(L, "uart read length must be 1-%d",
                          LUA_MODULE_UART_MAX_READ_LEN);
    }
...
{
    lua_module_uart_ud_t *ud = lua_module_uart_get_ud(L, 1);
    lua_Integer max_len = luaL_optinteger(L, 2, LUA_MODULE_UART_MAX_LINE_LEN);
    TickType_t ticks = lua_module_uart_timeout_ticks(L, 3);

    if (max_len <= 0 || max_len > LUA_MODULE_UART_MAX_LINE_LEN) {
        return luaL_error(L, "uart read_line max_len must be 1-%d",
                          LUA_MODULE_UART_MAX_LINE_LEN);
    }
...
static int lua_module_uart_write(lua_State *L)
{
    ...
    if (type == LUA_TSTRING) {
        ...
        if (n < 0 || n > LUA_MODULE_UART_MAX_READ_LEN) {
...
{
    lua_module_uart_ud_t *ud = (lua_module_uart_ud_t *)luaL_testudata(
        L, 1, LUA_MODULE_UART_METATABLE);
    if (ud && ud->installed) {
